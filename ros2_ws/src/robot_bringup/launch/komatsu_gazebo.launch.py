...
def generate_launch_description():
    """Generate all-in-one Gazebo + RViz + localization + mapping + Nav2 launch."""
    ...
    use_cmd_vel_joint_sim = DeclareLaunchArgument(
        ...
        default_value='false',
        description='Publish fake joint states from cmd_vel for RViz animation',
    )
    ...
    startup_logs = [
        LogInfo(msg=['[Action7] use_sim_odometry=', LaunchConfiguration('use_sim_odometry'),
                     ', use_sim_imu=', LaunchConfiguration('use_sim_imu'),
                     ', use_sim_scan=', LaunchConfiguration('use_sim_scan')]),
        LogInfo(msg=['[Action7] odom_topic=', LaunchConfiguration('odom_topic'),
                     ', scan_topic=', LaunchConfiguration('scan_topic')]),
    ]
