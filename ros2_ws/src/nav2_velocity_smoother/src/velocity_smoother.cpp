...
void VelocitySmoother::smootherTimer()
{
  ...
  // memorize linear.y of input as it is used for ETA computation and we need to reset it to 0.0 for that
  double distance_to_end_of_path = command_->linear.y;
  command_->linear.y = 0.0; //anyway, nav2_controller original node sets it to 0.0
  ...
  */ 
  RCLCPP_DEBUG(get_logger(), "Nav2_vel_smooth Dist end path : %.2f, Dist lookahead : %.2f, Dist cusp : %.2f, Curvature : %.2f", 
  distance_to_end_of_path, command_->linear.z, command_->angular.x, command_->angular.y);
  cmd_vel->linear.y = distance_to_end_of_path;
  cmd_vel->linear.z = command_->linear.z;
