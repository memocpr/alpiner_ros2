...
protected:
   ...
   */
  geometry_msgs::msg::PoseStamped getLookAheadPoint(const double &, const nav_msgs::msg::Path &, double* dist_to_end_of_path);
   ...
   * @param x Pointer to the x variable to store cusp.x
   * @param x Pointer to the y variable to store cusp.y
   * @return robot distance from the cusp
   */
  double findVelocitySignChange(const nav_msgs::msg::Path & transformed_plan, double * x, double* y);

  /**
   * @brief checks for the cusp position
   * @param pose Pose input to determine the cusp position
   * @return pose_id of the cusp in the global_plan_
   */
  int findCuspGlobalPlan(const nav_msgs::msg::Path & plan);
  ...
  /**
   * @brief Method to compute angle between two vectors
   * @param transformed_plan Transformed plan
   */
  double getAngleTwoVectors(const nav_msgs::msg::Path & transformed_plan);
  ...
  collision_checker_;
  int mem_mem_pose_id_closest_pose;
