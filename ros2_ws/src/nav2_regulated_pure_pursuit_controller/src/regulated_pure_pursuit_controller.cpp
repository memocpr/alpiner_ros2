...
namespace nav2_regulated_pure_pursuit_controller
{
      ...
      std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
  {
    ...
    this->mem_mem_pose_id_closest_pose = -1;
  }
      ...
      nav2_core::GoalChecker *goal_checker)
  {
    ...
    bool is_last_transformed_pose_end_of_path = false;
    auto transformed_plan = transformGlobalPlan(pose, &is_last_transformed_pose_end_of_path);

    // Find look ahead distance and point on path and publish
    double lookahead_dist = getLookAheadDistance(speed);

    // Cusp check
    // if cusp_x and cusp_y are modified, there is a cusp
    double dist_to_cusp = -1.0;
    double cusp_x = -1.0;
    double cusp_y = -1.0;
    dist_to_cusp = findVelocitySignChange(transformed_plan, &cusp_x, &cusp_y);
    if ((dist_to_cusp == 0.0) || (dist_to_cusp > 1000.0))
    {
      dist_to_cusp = -1.0; // we dont want 0.0 for dist to cusp
    }
    RCLCPP_INFO(logger_, "distance to cusp : %.2f, cusp.x : %.2f, cusp.y : %.2f", dist_to_cusp, cusp_x, cusp_y);

    // if the lookahead distance is further than the cusp, use the cusp distance instead
    if ((dist_to_cusp < lookahead_dist) && (dist_to_cusp != -1.0))
    {
      lookahead_dist = dist_to_cusp;
    }

    // get carrot pose and distance to end of transformed plan
    double distance_end_of_transformed_plan = -1.0;
    auto carrot_pose = getLookAheadPoint(lookahead_dist, transformed_plan, &distance_end_of_transformed_plan);
    carrot_pub_->publish(createCarrotMsg(carrot_pose));
    RCLCPP_INFO(logger_, "carrot pose (X;Y) : (%.4f;%.4f)", carrot_pose.pose.position.x, carrot_pose.pose.position.y);
    ...
    if (carrot_dist2 > 0.001)
    {
      ...
      RCLCPP_INFO(logger_, "Curvature of the path is : %.4f, and radius is : %.4f", curvature, 1.0/curvature);
    }

    // Make sure we dont steer too much when it is not required to do so
    // phi is the biggest angle between two vectors in the transformed plan
    // the vectors are made by two pairs of poses, end pair is 9 poses after the start pair
    double phi = getAngleTwoVectors(transformed_plan);
    RCLCPP_INFO(logger_, "max phi angle in transformed path : %.4f", phi);
    double max_curvature = 0.04; // means a 25 meters radius
    bool reduce_curvature_not_at_end = false;

    // first check if we are at the end of the path
    double distance_to_end_of_path_start_reducing_curvature = 4.0; // meters
    if (distance_end_of_transformed_plan < distance_to_end_of_path_start_reducing_curvature)
    {
      curvature = max_curvature * (distance_end_of_transformed_plan / (distance_to_end_of_path_start_reducing_curvature * 1.0));
      RCLCPP_INFO(logger_, "Reducing curvature to %.4f because of distance to end of path : %.4f", max_curvature, distance_end_of_transformed_plan);
    }

    // reduce curvature if we are near a cusp
    if (dist_to_cusp != -1.0 && dist_to_cusp < 2.0)
    {
      // if we are near a cusp, reduce curvature to max_curvature
      curvature = 0.06; // means a 16.67 meters radius
      RCLCPP_INFO(logger_, "Reducing curvature to %.4f because of distance to cusp : %.4f", curvature, dist_to_cusp);
    }

    // if not, check if we are in a straight line or not
    else if ((!(std::isnan(phi))) && (reduce_curvature_not_at_end == true))
    {
      double const PHI_MAX_DEG = 3.0; // in degrees
      // if phi angle in the path is in the range of [-3, 3] or [177, 183] degrees
      // then we can reduce the curvature
      if (((phi < PHI_MAX_DEG) && (phi > -1.0 * PHI_MAX_DEG)) || ((phi < 180.0 + PHI_MAX_DEG) && (phi > 180.0 - PHI_MAX_DEG)))
      {
        // now we reduce curvature if it has been computed and is too high
        // handle positive and negative curvature separately
        if (curvature >= 0.0 && curvature > max_curvature)
        {
          curvature = max_curvature;
          RCLCPP_INFO(logger_, "Reducing curvature to %.4f because of phi : %.4f", curvature, phi);
        }
        else if (curvature < 0.0 && curvature < (-1.0 * max_curvature))
        {
          curvature = (-1.0 * max_curvature);
          RCLCPP_INFO(logger_, "Reducing curvature to %.4f because of phi : %.4f", curvature, phi);
        }
      }
    }
    ...
    // our own data
    cmd_vel.twist.linear.y = distance_end_of_transformed_plan;
    cmd_vel.twist.linear.z = lookahead_dist;
    cmd_vel.twist.angular.x = dist_to_cusp;
    cmd_vel.twist.angular.y = curvature;
    return cmd_vel;
  }

  double RegulatedPurePursuitController::getAngleTwoVectors(
      const nav_msgs::msg::Path &transformed_plan)
  {
    int size_transformed_plan = transformed_plan.poses.size();
    RCLCPP_DEBUG(logger_, "numbers of points in transformed path : %d", size_transformed_plan);

    // make sure path is big enough
    if (transformed_plan.poses.size() > 10)
    {
      double max_phi = 0.0;

      // iterate over several points to find enough angles
      for (int i = 0; i < size_transformed_plan - 10; i++)
      {
        auto pose_a = transformed_plan.poses[i + 9];
        auto pose_b = transformed_plan.poses[i + 7];
        auto pose_c = transformed_plan.poses[i + 2];
        auto pose_d = transformed_plan.poses[i];
        double ab_x = pose_b.pose.position.x - pose_a.pose.position.x;
        double ab_y = pose_b.pose.position.y - pose_a.pose.position.y;
        double cd_x = pose_d.pose.position.x - pose_c.pose.position.x;
        double cd_y = pose_d.pose.position.y - pose_c.pose.position.y;
        double res = ab_x * cd_x + ab_y * cd_y;
        double norm_ab = sqrt(ab_x * ab_x + ab_y * ab_y);
        double norm_cd = sqrt(cd_x * cd_x + cd_y * cd_y);
        double cos_phi = res / (norm_ab * norm_cd);
        double phi = acos(cos_phi) * 180.0 / 3.14159265358979323846264;
        if (phi > max_phi){
          max_phi = phi;
        }
      }
      RCLCPP_INFO(logger_, "max_phi angle in transformed path : %.4f", max_phi);
      return max_phi;
    }
  ...
  geometry_msgs::msg::PoseStamped RegulatedPurePursuitController::getLookAheadPoint(
      ...
      const nav_msgs::msg::Path &transformed_plan,
      double *dist_to_end_of_path)
  {

    // compute distance to end of transformed path
    auto end_position = transformed_plan.poses[transformed_plan.poses.size() - 2].pose.position;
    if ((end_position.x > 10000.0) || (end_position.y > 10000.0))
    {
      *dist_to_end_of_path = -1.0;
      RCLCPP_WARN(logger_, "Computing distance to end of path (%.2f, %.2f) -> wrong coordinate...", end_position.x, end_position.y);
    }
    else
    {
      *dist_to_end_of_path = hypot(end_position.x, end_position.y);
      RCLCPP_DEBUG(logger_, "Computing distance to end of path (%.2f, %.2f) -> %.2f meters away", end_position.x, end_position.y, *dist_to_end_of_path);
      *dist_to_end_of_path = ((*dist_to_end_of_path > 50.0) || ((*dist_to_end_of_path > -0.001) && (*dist_to_end_of_path < 0.001))) ? -1.0 : (*dist_to_end_of_path);
    }
  ...
  nav_msgs::msg::Path RegulatedPurePursuitController::transformGlobalPlan(
      const geometry_msgs::msg::PoseStamped &pose,
      bool *is_last_transformed_pose_end_of_path)
  {
    ...
    // find cusp
    // if there is a cusp, we must stay before cusp
    int pose_id_cusp = -1;
    pose_id_cusp = findCuspGlobalPlan(global_plan_);
    double dist_to_cusp_global_plan = -1.0;
    if (pose_id_cusp != -1)
    {
      dist_to_cusp_global_plan = euclidean_distance(robot_pose, global_plan_.poses[pose_id_cusp]);
      RCLCPP_INFO(logger_, "Found a cusp in global plan at distance : %.3f", dist_to_cusp_global_plan);
    }

    // replace max_robot_pose_search_dist_ by dist to cusp if there is a cusp
    // we want to stay on the same branch
    bool flag_cusp_closer = false;
    double dist_search_closest_pose_upper_bound = max_robot_pose_search_dist_;
    if ((dist_to_cusp_global_plan != -1.0) && (dist_to_cusp_global_plan < max_robot_pose_search_dist_))
    {
      dist_search_closest_pose_upper_bound = dist_to_cusp_global_plan;
      flag_cusp_closer = true;
      RCLCPP_INFO(logger_, "Replacing max_robot_pose_search with cusp distance -> %.3f", dist_search_closest_pose_upper_bound);
    }

    // Get the pose that is just after the max_robot_pose_search_dist parameter, or at the cusp
    auto closest_pose_upper_bound =
        nav2_util::geometry_utils::first_after_integrated_distance(
            global_plan_.poses.begin(), global_plan_.poses.end(), dist_search_closest_pose_upper_bound);
    RCLCPP_DEBUG(logger_, "closest_pose_upper_bound in global plan frame : (%.2f, %.2f), dist_search_closest_pose_upper_bound : %.2f ",
                closest_pose_upper_bound->pose.position.x, closest_pose_upper_bound->pose.position.y, dist_search_closest_pose_upper_bound);

    // First find the closest pose on the path to the robot
    // bounded by when the path turns around (if it does) so we don't get a pose from a later
    // portion of the path
    double mem_smallest_distance = 1000000.0;
    int mem_pose_id = 0;
    unsigned int upper_limit = closest_pose_upper_bound - global_plan_.poses.begin();
    for (unsigned int pose_id = 0; pose_id < upper_limit; pose_id++)
    {
      auto ps = global_plan_.poses[pose_id];
      double dist = euclidean_distance(robot_pose, ps);
      if (dist < mem_smallest_distance)
      {
        mem_smallest_distance = dist;
        mem_pose_id = pose_id;
      }
    }
    int additional_offset_pose_id = 0;
    if (this->mem_mem_pose_id_closest_pose != -1)
    {
      if ((this->mem_mem_pose_id_closest_pose == mem_pose_id) && (flag_cusp_closer == true))
      {
        additional_offset_pose_id += 5;
      }
    }
    this->mem_mem_pose_id_closest_pose = mem_pose_id;

    // get the beginning of the path transform
    auto transformation_begin = global_plan_.poses.begin();
    if (transformation_begin + mem_pose_id + additional_offset_pose_id <= global_plan_.poses.end())
    {
      transformation_begin += mem_pose_id + additional_offset_pose_id;
    }
    ...
    */
    ...
    if (transformation_end == global_plan_.poses.end())
    {
      *is_last_transformed_pose_end_of_path = true;
    }
      ...
      const geometry_msgs::msg::PoseStamped &pose)
  {
    ...
    {
      dist_to_cusp_global_plan = euclidean_distance(robot_pose, global_plan_.poses[pose_id_cusp]);
      RCLCPP_DEBUG(logger_, "Found a cusp in global plan at distance : %.3f", dist_to_cusp_global_plan);
    }
  ...
  double RegulatedPurePursuitController::findVelocitySignChange(
      const nav_msgs::msg::Path &transformed_plan, double *x, double *y)
  {
    // Iterating through the transformed global path to determine the position of the cusp
    // always returning the first cusp
    for (unsigned offset_search = 3; offset_search > 0; offset_search -= 1)
    {
      if (offset_search > transformed_plan.poses.size() - 1)
      {
        continue;
      }
      for (unsigned int pose_id = offset_search; pose_id < transformed_plan.poses.size() - offset_search; pose_id += offset_search)
      {
        // We have two vectors for the dot product OA and AB. Determining the vectors.
        double oa_x = transformed_plan.poses[pose_id].pose.position.x -
                      transformed_plan.poses[pose_id - offset_search].pose.position.x;
        double oa_y = transformed_plan.poses[pose_id].pose.position.y -
                      transformed_plan.poses[pose_id - offset_search].pose.position.y;
        double ab_x = transformed_plan.poses[pose_id + offset_search].pose.position.x -
                      transformed_plan.poses[pose_id].pose.position.x;
        double ab_y = transformed_plan.poses[pose_id + offset_search].pose.position.y -
                      transformed_plan.poses[pose_id].pose.position.y;

        //  normally, dot product must be > 0.0 to get an angle < 90°, but one vector is in the opposite direction as the convention.
        ...
        if ((oa_x * ab_x) + (oa_y * ab_y) < 0.0)
        {
          ...
          // cusp position returned via pointers
          *x = transformed_plan.poses[pose_id].pose.position.x;
          *y = transformed_plan.poses[pose_id].pose.position.y;
          return hypot(
              transformed_plan.poses[pose_id].pose.position.x,
              transformed_plan.poses[pose_id].pose.position.y);
        }
  ...
  int RegulatedPurePursuitController::findCuspGlobalPlan(const nav_msgs::msg::Path &plan)
  {
    // Iterating through the global path to determine the position of the cusp
    // always returning the first cusp
    for (int offset_search = 3; offset_search > 0; offset_search -= 1)
    {
      for (unsigned int pose_id = offset_search; pose_id < plan.poses.size() - offset_search; pose_id += offset_search)
      {
        // We have two vectors for the dot product OA and AB. Determining the vectors.
        double oa_x = plan.poses[pose_id].pose.position.x -
                      plan.poses[pose_id - offset_search].pose.position.x;
        double oa_y = plan.poses[pose_id].pose.position.y -
                      plan.poses[pose_id - offset_search].pose.position.y;
        double ab_x = plan.poses[pose_id + offset_search].pose.position.x -
                      plan.poses[pose_id].pose.position.x;
        double ab_y = plan.poses[pose_id + offset_search].pose.position.y -
                      plan.poses[pose_id].pose.position.y;

        //  normally, dot product must be > 0.0 to get an angle < 90°, but one vector is in the opposite direction as the convention.
        //  therefore this is right
        if ((oa_x * ab_x) + (oa_y * ab_y) < 0.0)
        {
          // returning the pose_id if there is a cusp
          return pose_id;
        }
      }
    }
