...
void ControllerServer::computeAndPublishVelocity()
{
  ...
  try {
    // call to our own nav2_regulated_pure_pursuit_controller with our own custom data as follow
    /* our own data
    cmd_vel.twist.linear.y = are_we_at_the_end;
    ...
    */ 
    ...
    last_valid_cmd_time_ = now();
    //RCLCPP_INFO(this->get_logger(), "Received this curvature : %.4f, dist to cusp : %.4f and dist to lookahead point : %.4f", cmd_vel_2d.twist.angular.y, cmd_vel_2d.twist.linear.y, cmd_vel_2d.twist.linear.z);
  } catch (nav2_core::PlannerException & e) {
...
void ControllerServer::publishVelocity(const geometry_msgs::msg::TwistStamped & velocity)
{
  ...
  /* in nav2_regulated_pure_pursuit, we have more custom data
  cmd_vel.twist.linear.y = distance_end_of_transformed_plan;
  cmd_vel.twist.linear.z = lookahead_dist;
  cmd_vel.twist.angular.x = dist_to_cusp;
  cmd_vel.twist.angular.y = curvature;
  */
  cmd_vel->angular.x = velocity.twist.angular.x;
  ...
  RCLCPP_DEBUG(get_logger(), "Nav2_controller Dist end path : %.2f, Dist lookahead : %.2f, Dist cusp : %.2f, Curvature : %.2f", 
    cmd_vel->linear.y, cmd_vel->linear.z, cmd_vel->angular.x, cmd_vel->angular.y);
